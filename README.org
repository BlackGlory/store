...
删除时可用 =If-Match= 包含revision, 防止幻影删除, 不匹配时返回412.
...
**** Revision策略
设置环境变量 =UPDATE_REVISION_REQUIRED=true= 可强制要求客户端在发出更新请求时提供 =If-Match= 请求头, 默认为 =false=.
设置环境变量 =DELETE_REVISION_REQUIRED=true= 可强制要求客户端在发出删除请求时提供 =If-Match= 请求头, 默认为 =false=.

...
****** 获取JSON Schema
=GET /admin/store/<id>/json-schema=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/store/$id/json-schema"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/json-schema`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
****** 设置JSON Schema
=PUT /admin/store/<id>/json-schema=
...
curl \
  ...
  --header "Content-Type: application/json" \
  --data "$JSON_SCHEMA" \
  "http://localhost:8080/admin/store/$id/jsonschema"
#+END_SRC
...
****** 移除JSON Schema
=DELETE /admin/store/<id>/json-schema=
...
=GET /admin/store/<id>/revision-policies=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/store/$id/revision-policies"
#+END_SRC

******* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/revision-policies`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
***** 设置revision策略
=PUT /admin/store/<id>/revision-policies/update-revision-required=
=PUT /admin/store/<id>/revision-policies/delete-revision-required=
...
curl \
  ...
  "http://localhost:8080/admin/store/$id/revision-policies/update-revision-required"
#+END_SRC

******* fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/revision-policies/update-revision-required`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  , 'Content-Type': 'application/json'
...
***** 移除revision策略
=DELETE /admin/store/<id>/revision-policies/update-revision-required=
=DELETE /admin/store/<id>/revision-policies/delete-revision-required=
...
****** 获取特定频道的token策略
=GET /admin/store/<id>/token-policies=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/store/$id/token-policies"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fethc(`http://localhost:8080/admin/store/${id}/token-policies`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
****** 设置token策略
=PUT /admin/store/<id>/token-policies/write-token-required=
=PUT /admin/store/<id>/token-policies/read-token-required=
=PUT /admin/store/<id>/token-policies/delete-token-required=
...
curl \
  ...
  "http://localhost:8080/admin/store/$id/token-policies/write-token-required"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/token-policies/write-token-required`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  , 'Content-Type': 'application/json'
...
****** 移除token策略
=DELETE /admin/store/<id>/token-policies/write-token-required=
=DELETE /admin/store/<id>/token-policies/read-token-required=
=DELETE /admin/store/<id>/token-policies/delete-token-required=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/store/$id/tokens"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/tokens`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
=PUT /admin/store/<id>/tokens/<token>/write=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/store/$id/tokens/$token/log"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/tokens/${token}/log`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/store/<id>/tokens/<token>/write=
...
curl \
  --request DELETE \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/store/$id/tokens/$token/write"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/tokens/${token}/write`, {
  method: 'DELETE'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=PUT /admin/store/<id>/tokens/<token>/read=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/store/$id/tokens/$token/read"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/tokens/${token}/read`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/store/<id>/tokens/<token>/read=
...
=PUT /admin/store/<id>/tokens/<token>/delete=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/store/$id/tokens/$token/delete"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/store/${id}/tokens/${token}/delete`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/store/<id>/tokens/<token>/delete=
